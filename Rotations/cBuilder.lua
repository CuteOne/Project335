...
function br.loader:new(spec,specName)
    ...
    for unitClass , classTable in pairs(br.lists.spells) do
        if unitClass == select(2,UnitClass('player')) or unitClass == 'Shared' then
            for spec, specTable in pairs(classTable) do
                if spec == br.currentSpec or spec == 'Shared' then
    ...
    local function getTalentInfo()
        br.activeSpecGroup = 0 --GetActiveTalentGroup()
    ...
    cframe:RegisterEvent("PLAYER_TALENT_UPDATE")
    -- Update Talent Info
    ...
    function self.getSnapshotValue(dot)
        -- Feral Bleeds
        if GetTalentTabInfo(GetPrimaryTalentTree()) == 103 then
