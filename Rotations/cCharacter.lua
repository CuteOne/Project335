...
-- Update Character Stats
	function self.getCharacterInfo()
		...
		self.gcdMax 			= 1.5 --max(1, 1.5 / (1 + UnitSpellHaste("player") / 100))
		self.health 			= getHP("player")
		self.instance 			= select(2,IsInInstance())
		self.level 				= UnitLevel("player") -- TODO: EVENT - UNIT_LEVEL
		self.spec 				= "None" --select(2, GetTalentTabInfo(GetPrimaryTalentTree())) or "None"
...
-- Returns the Global Cooldown time
	function self.getGlobalCooldown()
        ...
        local gcdMIN = 0.75
        if UnitClass("player") == "Druid" or UnitClass("player") == "Rogue" then
        	return 1
        else
