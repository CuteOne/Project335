...
if not metaTable1 then
	...
	metaTable1.__call = function(_, ...) -- (_, forceRetable, excludePets, onlyInRange) [Not Implemented]
		...
		local groupSize = IsInRaid() and GetNumRaidMembers() or GetNumRaidMembers() - 1 -- If in raid, we check the entire raid. If in party, we remove one from max to account for the player.
