...
function getTalent(Row,Column,specGroup)
	if specGroup == nil then specGroup = GetActiveTalentGroup() end
...
function isValidUnit(Unit)
	...
	then
		local instance = IsInInstance() ~= "none"
		local distance = getDistance(Unit,"player")
		local inCombat = UnitAffectingCombat("player")~=nil or (GetObjectExists("pet") and UnitAffectingCombat("pet")~=nil)
		local hasThreat = hasThreat(Unit) or isTargeting(Unit) or (GetObjectExists("pet") and (hasThreat(Unit,"pet") or isTargeting(Unit,"pet")))--[[ or isBurnTarget(Unit) > 0--]]
		local playerTarget = UnitIsUnit(Unit,"target")
		local validUnit = hasThreat or (not instance and playerTarget) or (instance and (#br.friend == 1 or inCombat) and playerTarget) or (isDummy(Unit) and distance < 8)
		if validUnit == 1 or validUnit == true then return true end
	end
	return false
end
...
function pause(skipCastingCheck)
	...
	if (pausekey and GetCurrentKeyBoardFocus() == nil and isChecked("Pause Mode"))
		...
		or UnitBuffID("player",433) -- Eating
