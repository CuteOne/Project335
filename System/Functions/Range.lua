function getDistance(Unit1,Unit2,option)
    ...
    if GetObjectExists(Unit1) and GetUnitIsVisible(Unit1) ~= nil
        and GetObjectExists(Unit2) and GetUnitIsVisible(Unit2) ~= nil
    then
    -- Get the distance
        ...
        local dist = math.sqrt(((X2-X1)^2) + ((Y2-Y1)^2) + ((Z2-Z1)^2)) - (PlayerCombatReach + TargetCombatReach)
        local meleeRange = max(5, PlayerCombatReach + TargetCombatReach + MeleeCombatReachConstant + IfSourceAndTargetAreRunning)
        if dist > meleeRange then -- Thanks Ssateneth
            currentDist = dist
        else
            currentDist = 0
        end
