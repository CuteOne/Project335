...
function frame:OnEvent(event, arg1, arg2, arg3, arg4, arg5)
    ...
    if event == "PLAYER_ENTERING_WORLD" then
        ...
        br.selectedSpec = br.currentSpec
        br.activeSpecGroup = GetActiveTalentGroup()
			...
			if FireHack == nil then
				...
				if br.data.settings ~= nil then
					if br.data.settings[br.selectedSpec].toggles["Power"] ~= nil and br.data.settings[br.selectedSpec].toggles["Power"] ~= 1 then
						...
						-- for i=1, 40 do
						-- 	local _,_,_,_,_,_,_,_,_,_,buffSpellID = UnitBuff("player",i)
						-- 	if buffSpellID ~= nil then
						-- 		return print(buffSpellID)
						-- 	end
						-- end
					-- Blizz CastSpellByName bug bypass
						if castID then
					...
					-- Load Spec Profiles
					    ...
					    local playerSpec = br.selectedSpec --GetTalentTabInfo(GetPrimaryTalentTree())
					    -- Initialize Player
					...
					-- Close windows and swap br.selectedSpec on Spec Change
						if br.currentSpec ~= br.selectedSpec then
